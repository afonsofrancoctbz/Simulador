                ...
                {scenariosToShow.map((scenario) => {
                    ...
                    >
                                            ...
                                            return (
                                                <div key={item.name} className={cn("flex justify-between items-center text-sm", isTestTax ? "bg-amber-50/50 p-1.5 -mx-1.5 rounded text-muted-foreground" : "")}>
                                                    <span className="text-foreground flex items-center gap-1.5">
                                                        {isTestTax && (
                                                            <TooltipProvider><Tooltip><TooltipTrigger><HelpCircle className="h-3.5 w-3.5 text-amber-500 cursor-help" /></TooltipTrigger><TooltipContent className="max-w-[250px]"><p>Este valor é um teste de adaptação do sistema tributário (2026). Ele é <strong>totalmente compensado (descontado)</strong> do PIS/COFINS e não aumenta o custo final da sua empresa neste ano.</p></TooltipContent></Tooltip></TooltipProvider>
                                                        )}
                                                        ...
                                                        {showIvaMemory && (<TooltipProvider><Tooltip><TooltipTrigger><Calculator className="h-3.5 w-3.5 text-muted-foreground/70 cursor-help hover:text-primary transition-colors" /></TooltipTrigger><TooltipContent className="p-4 bg-card border shadow-xl max-w-xs"><div className="space-y-2 text-xs"><p className="font-bold text-primary mb-2 border-b pb-1">Memória de Cálculo (IVA)</p><div className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-1">{(() => { const baseRevenue = scenario.domesticRevenue ?? scenario.totalRevenue; const expenses = formValues.creditGeneratingExpenses; const netBase = Math.max(0, baseRevenue - expenses); const impliedRate = netBase > 0 ? item.value / netBase : 0; return ( <><span className="text-muted-foreground">Faturamento Base:</span><span className="font-mono">{formatCurrencyBRL(baseRevenue)} - {formatPercent(impliedRate)}</span><span className="text-green-600">(-) Crédito Despesa:</span><span className="font-mono text-green-600">{formatCurrencyBRL(expenses)} - {formatPercent(impliedRate)}</span><div className="col-span-2 border-t my-1"></div><span className="font-semibold text-foreground">Base Líquida:</span><span className="font-mono font-bold">{formatCurrencyBRL(netBase)}</span><span className="font-semibold text-foreground">Impostos Liquido:</span><span className="font-mono font-bold">{formatCurrencyBRL(item.value)}</span></> )})()}</div><p className="text-[10px] text-muted-foreground mt-2 italic">*O imposto incide sobre o valor agregado (Base Líquida x Alíquota).</p></div></TooltipContent></Tooltip></TooltipProvider>)}
                                                        {showLpMemory && (<TooltipProvider><Tooltip><TooltipTrigger><Calculator className="h-3.5 w-3.5 text-muted-foreground/70 cursor-help hover:text-primary transition-colors" /></TooltipTrigger><TooltipContent className="p-4 bg-card border shadow-xl max-w-xs"><div className="space-y-2 text-xs"><p className="font-bold text-primary mb-2 border-b pb-1">Memória de Cálculo ({item.name.split(' ')[0]})</p><div className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-1">{(() => { const { baseIRPJ, baseCSLL } = getPresumedBases; const rawBase = item.name.includes("IRPJ") ? baseIRPJ : baseCSLL; const totalRev = scenario.totalRevenue; const standardPresuRate = totalRev > 0 ? rawBase / totalRev : 0; const LIMIT_MAJORATION_MENSAL = 1250000 / 3; const hasMajoration = totalRev > LIMIT_MAJORATION_MENSAL; let calculatedBase = 0; let baseNormal = 0; let baseExcess = 0; if (hasMajoration) { baseNormal = LIMIT_MAJORATION_MENSAL * standardPresuRate; baseExcess = (totalRev - LIMIT_MAJORATION_MENSAL) * (standardPresuRate * 1.10); calculatedBase = baseNormal + baseExcess; } else { calculatedBase = rawBase; } const irpjBasic = calculatedBase * 0.15; const irpjAdd = Math.max(0, calculatedBase - 20000) * 0.10; const csll = calculatedBase * 0.09; return ( <>{hasMajoration ? (<><div className="col-span-2 my-1 border-t border-dashed opacity-50"></div><span className="text-muted-foreground">Base (Até R$ 416k):</span><span className="font-mono">{formatCurrencyBRL(baseNormal)} <span className="text-[10px] ml-1 opacity-70">({formatPercent(standardPresuRate).replace(',00','')})</span></span><span className="text-amber-700 font-medium">Base (Excedente):</span><span className="font-mono text-amber-700 font-medium">{formatCurrencyBRL(baseExcess)}<span className="text-[10px] ml-1">({formatPercent(standardPresuRate * 1.10)})</span></span><div className="col-span-2 border-t my-1"></div><span className="font-semibold text-foreground">Base Total Calculada:</span><span className="font-mono font-bold">{formatCurrencyBRL(calculatedBase)}</span></>) : (<><span className="text-foreground">Base de Cálculo ({formatPercent(standardPresuRate).replace(',00', '')}):</span><span className="font-mono font-semibold">{formatCurrencyBRL(calculatedBase)}</span><div className="col-span-2 border-t my-1"></div></>)}{item.name.includes("IRPJ") ? (<><span className="text-muted-foreground">Alíquota (15%):</span><span className="font-mono">{formatCurrencyBRL(irpjBasic)}</span>{irpjAdd > 0 && (<><span className="text-muted-foreground">Adicional (10%):</span><span className="font-mono">{formatCurrencyBRL(irpjAdd)}</span></>)}</>) : (<><span className="text-muted-foreground">Alíquota (9%):</span><span className="font-mono">{formatCurrencyBRL(csll)}</span></>)}{totalRev > LIMIT_MAJORATION_MENSAL && (<div className="col-span-2 mt-2 text-[10px] text-amber-600 leading-tight">* Base majorada em 10% sobre o excedente de R$ 416k (IN 2.306/2026).</div>)}</> )})()}</div></div></TooltipContent></Tooltip></TooltipProvider>)}
                                                    </span>
                        ...
                        <div className="p-4 mt-auto space-y-2 bg-muted/30 rounded-b-xl">
                            {fatorRProjection && !fatorRProjection.isEnquadradoAgora && projectionNote && (<Alert variant="default" className={cn("p-3", {'bg-green-100/80 border-green-200/80 text-green-900': projectionStatus === 'success','bg-amber-100/80 border-amber-200/80 text-amber-900': projectionStatus === 'warning' || projectionStatus === 'error','bg-primary/10 border-primary/20': !projectionStatus || projectionStatus === 'info'})}><AlertDescription className="text-xs font-medium flex items-start gap-2"><span className={cn('text-primary/90', {'text-green-600': projectionStatus === 'success','text-amber-600': projectionStatus === 'warning' || projectionStatus === 'error'})}><BadgeInfo className="h-4 w-4 mt-0.5 shrink-0" /></span><span className={cn('text-primary/90', {'text-green-900': projectionStatus === 'success','text-amber-900': projectionStatus === 'warning' || projectionStatus === 'error'})} dangerouslySetInnerHTML={{ __html: projectionNote.replace(/\\n/g, '<br/>') }}></span></AlertDescription></Alert>)}
                            {scenario.regime.includes('Híbrido') && (<Alert variant="default" className="bg-amber-100/80 border-amber-200/80 text-amber-900 p-3"><AlertDescription className="text-xs font-medium flex items-start gap-2"><Info className="h-4 w-4 mt-0.5 shrink-0" /><span>Cenário B2C: O IVA pago por fora não gera benefício ao cliente final (pessoa física), elevando a carga total.</span></AlertDescription></Alert>)}
                            {scenario.optimizationNote && !projectionNote && !scenario.regime.includes('Híbrido') && (<Alert variant="default" className="bg-primary/10 border-primary/20 text-primary-foreground p-3"><AlertDescription className="text-xs text-primary/90 font-medium flex items-start gap-2"><Info className="h-4 w-4 mt-0.5 shrink-0"/><span>{scenario.optimizationNote}</span></AlertDescription></Alert>)}
                            {scenario.notes.length > 0 && (<Alert variant="default" className="bg-primary/10 border-primary/20 text-primary-foreground p-3"><AlertDescription className="text-xs text-primary/90 font-medium flex items-start gap-2"><Info className="h-4 w-4 mt-0.5 shrink-0"/><span>{scenario.notes.join(' ')}</span></AlertDescription></Alert>)}
