...
export default function FatorRPage() {
    ...
    const handleMonthlyDataChange = (index: number, field: 'receita' | 'folha', value: number) => {
        ...
        const updatedData = [...dadosMensais];
        updatedData[index] = { ...updatedData[index], [field]: value };
        setDadosMensais(updatedData);

        const newRbt12 = updatedData.reduce((acc, item) => acc + (item.receita || 0), 0);
        const newFolha12 = updatedData.reduce((acc, item) => acc + (item.folha || 0), 0);
        form.setValue('rbt12', newRbt12);
        form.setValue('folha12', newFolha12);
    };
