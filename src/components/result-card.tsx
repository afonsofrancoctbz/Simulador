...
const ResultCardComponent = ({ details, isCheapest, formValues }: { details: TaxDetails, isCheapest: boolean, formValues: TaxFormValues }) => {
    ...
    return (
        <Card className={cn(
            ...
            <CardContent className="flex-grow p-3 space-y-3 flex flex-col">
                
                <div className="p-2.5 border rounded-lg bg-background/80 space-y-1.5 text-sm">
                ...
                <div className="space-y-3">
                     <div className="p-2.5 border rounded-lg bg-background/80 space-y-2 text-sm">
                        
                        {faturamentoTaxes.length > 0 && (
                             <div className="space-y-1.5">
                                <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1">Impostos s/ Faturamento</h4>
                                    ...
                                    <div key={index} className="flex justify-between items-center border-b border-dashed pb-1 last:border-b-0">
                                        <span className="text-muted-foreground text-sm">
                                            ...
                                            {item.name === 'DAS (Guia Unificada)' && details.effectiveDasRate !== undefined && (
                                                <span className='ml-1.5 font-bold text-accent'>({formatPercent(details.effectiveDasRate)})</span>
                                            )}
                                        </span>
                        ...
                        {folhaTaxes.length > 0 && (
                             <div className="space-y-1.5 pt-2">
                                <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1">Encargos s/ Folha e Pró-labore</h4>
                        ...
                        <div className="space-y-1.5 pt-2">
                            <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1">Outros Custos</h4>
                             <div className="flex justify-between items-center">
                                <span className="text-muted-foreground text-sm">Mensalidade Contabilizei</span>
                 ...
                 {details.fatorR !== undefined && (
                    <div className={cn(
                        ...
                        <span className="font-semibold">Fator R: <span className="font-bold">{formatPercent(details.fatorR)}</span></span>
                        <p className="text-xs mt-1">{details.fatorR >= 0.28 ? '✅ Cenário otimizado' : '⚠️ Alíquota maior aplicada'}</p>
                    </div>
                )}

                 {details.optimizationNote && (
                    <div className="p-2.5 rounded-lg bg-emerald-50 text-emerald-900 border border-emerald-200 text-xs flex items-start gap-2">
