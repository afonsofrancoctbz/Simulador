...
const taxOptimizationAdvicePrompt = ai.definePrompt({
  ...
  **Análise Mandatória:**
  ...
  3.  **Receitas de Exportação (Análise Crítica):** Se houver receitas de exportação (\`totalExportRevenue > 0\`), este é o ponto MAIS IMPORTANTE da análise. Enfatize que a prestação de serviços para o exterior tem IMUNIDADE de PIS, COFINS e ISS, o que reduz drasticamente a carga tributária. Esclareça que, embora IRPJ e CSLL ainda sejam devidos, a economia gerada pela isenção dos outros tributos é imensa, tornando a exportação de serviços uma vantagem competitiva significativa.
  4.  **Pró-labore vs. Distribuição de Lucros:** Diferencie o pró-labore da distribuição de lucros. Explique que o pró-labore é a remuneração do sócio, com retenção de 11% de INSS e IRRF. Além disso, no Lucro Presumido e no Simples Nacional (Anexo IV), a empresa tem o custo da Contribuição Previdenciária Patronal (20% + adicionais) sobre o pró-labore. Já a distribuição de lucros é isenta de impostos para o sócio. Se a otimização do Fator R não for o principal fator, sugira manter um pró-labore estratégico (pelo menos um salário mínimo) e retirar o restante como lucros para maior eficiência tributária.
