...
const taxOptimizationAdvicePrompt = ai.definePrompt({
  ...
  **Análise Mandatória:**
  ...
  3.  **Receitas de Exportação:** Se houver receitas de exportação (\`totalExportRevenue > 0\`), trate isso como um ponto CRÍTICO da análise. Explique de forma clara que a prestação de serviços para o exterior goza de imunidade tributária para PIS, COFINS e ISS. Destaque que essa isenção reduz significativamente a carga tributária e é um dos principais benefícios para empresas que exportam serviços.
